...
title Optimizar Windows 10 - TODO EN UNO
color 0B

REM ============================================================
REM  Optimizar Windows 10 - ARCHIVO UNICO (todo incluido)
REM  Doble clic para ejecutar TODO automaticamente.
REM  - Se auto-eleva a Administrador (di SI en el aviso UAC).
REM  - No necesita el .ps1 aparte, ni bash, ni WSL.
REM ============================================================
...
set "SELF=%~f0"
set "OUT=%TEMP%\Optimizar-Windows10.ps1"

echo ============================================================
echo   OPTIMIZAR WINDOWS 10 - Ejecucion automatica
echo   No cierres esta ventana hasta que termine.
echo ============================================================
echo.

REM --- Extraer el PowerShell embebido a un archivo temporal ---
powershell -NoProfile -ExecutionPolicy Bypass -Command "$t=[IO.File]::ReadAllText($env:SELF,[Text.Encoding]::UTF8); $m=('#@PS'+'_BEGIN@'); $i=$t.IndexOf($m); if($i -lt 0){Write-Host 'Error: marcador no encontrado'; exit 1}; $ps=$t.Substring($i+$m.Length); [IO.File]::WriteAllText($env:OUT,$ps,(New-Object Text.UTF8Encoding($false)))"

if not exist "%OUT%" (
    echo Error al preparar el script. Abortando.
    pause
    exit /b 1
)

REM --- Ejecutar en modo automatico ---
powershell -NoProfile -ExecutionPolicy Bypass -File "%OUT%" -Auto

echo.
echo ============================================================
echo   Proceso terminado. Se recomienda REINICIAR el equipo.
echo ============================================================
pause
exit /b
